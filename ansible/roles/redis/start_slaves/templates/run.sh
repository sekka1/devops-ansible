...
-d '{
  ...
  "options": [
    ...
    },
    {
      "section": "Service",
      "name": "ExecStart",
      "value": "/usr/bin/docker run --name '${DOCKER_NAME}' -v {{ redis_data_directory }}:/data -p {{ EXPOSED_REDIS_PORT }}:6379 {{ docker_image_redis }} redis-server --slaveof db1.navabit.com 6379 --appendonly yes"
    },
    {
      "section": "Service",
      "name": "ExecStop",
